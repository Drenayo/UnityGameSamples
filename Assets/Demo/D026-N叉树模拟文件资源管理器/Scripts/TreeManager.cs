...
namespace D026
{
    public class TreeManager : MonoBehaviour
    {
        ...
        private Stack<Tree.Node> pathStack = new Stack<Tree.Node>();
        // 存储
        ...
        // 删除Item *
        public void DeletItem(Tree.Node node)
        {
            Debug.Log($"删除{node.data}");
            
            Tree.Node parentNode = tree.FindParentNode(node);
            Tree.Node delNode = tree.RemoveNode(node);
            RefUI(parentNode);
        ...
        // 移动Item *
        public void MoveItem(Tree.Node node)
        {
        ...
        // 重命名Item Done
        public void RenameItem(Tree.Node node)
        {
        ...
        // 新建Item
        private void CreateItem(Tree.Node parentNode)
        {
            ...
            {
                // 添加节点
                Tree.Node node = new Tree.Node(obj.GetComponentInChildren<InputField>().text);
        ...
        // 刷新该节点的UI视图 该层被刷新
        private void RefUI(Tree.Node node)
        {
            //Debug.Log("刷新UI:" + node.data + "子列表");
            foreach (Transform item in svContent)
                Destroy(item.gameObject);

            foreach (Tree.Node n in node.childList)
            {
        ...
        // 刷新路径
        private void RefPath(Tree.Node node, int i)
        {
        ...
        // 测试遍历栈
        public void PrintStackTest(Stack<Tree.Node> stack)
        {
            string str = string.Empty;
            foreach (Tree.Node node in stack)
            {
                str += "<-" + node.data;
            }
            Debug.Log(str);
        ...
        private void Push(Tree.Node node)
        {
            pathStack.Push(node);
        ...
        private void Pop(Tree.Node node)
        {
