...
namespace D023
{
    ...
    public class SortManager : MonoBehaviour
    {
        ...
        // 希尔排序
        IEnumerator ShellSort_Y()
        {
            Debug.Log("希尔排序执行");
            int gap = 1;
            while (gap < dataList.Count / 3)
            {
                gap = gap * 3 + 1;
            }
            ...
            while (gap >= 1)
            {
                for (int i = gap; i < dataList.Count; i++)
                {
                    for (int j = i; j >= gap; j -= gap)
                    {
                        entityList[j].GetComponent<Item>().SetColor(sortSetting.currColor);
                        entityList[j - gap].GetComponent<Item>().SetColor(sortSetting.currColor);
                        yield return new WaitForSeconds(sortSetting.SortSpeed);

                        if (dataList[j] < dataList[j - gap])
                        {
                            Swap(j, j - gap);
                        }
                        else
